...
{
echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>Thoải mái đi<br /><form method="get">Url: <input name="url" type="text"><br />Bỏ trống để leech hết truyện. Nhập trang đầu và kết thúc cách nhau dấu chấm vd:"0.3"<br /> <input name="pt" type="text"><input type="submit" value="Leech" ></form>';
}
elseif ($_POST['submit'])
{
...
$title = trim($title[0]);
$lay = explode('<a class="last" href="'.$url.'/',$laya);
$lay = explode('">»</a>',$lay[1]);
$lay = trim($lay[0]);
if (!$lay){
$lay = explode("<div class='wp-pagenavi'>",$laya);
$lay = explode('<a class="nextpostslink"',$lay[1]);
$lay = trim($lay[0]);
$lay = strip_tags($lay,'');
$lay = substr($lay, -1 , 1 );
}
...
$bai = curl_exec($bv);
$bai = explode('<div class="bai-viet-box">',$bai);
$bai = explode('<div class="chiase">',$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai,'<p>,<br>,<b>,<i>,<u>,<strong>');
$bai = preg_replace('/(hayso1.vn|hayso1)/i', '', $bai);
$bai = str_replace('</p><p>','',$bai);
$bai = preg_replace('/<p>(Chap|Chương|Phần)(.*)<\/p>/', '<p><b>$1$2</b></p>', $bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
