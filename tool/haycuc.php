...
{
echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><form method="get">Url: <input name="url" type="text"><br />(Dành cho truyện)Bỏ trống để leech full truyện. nếu muốn leech từng trang thì nhập VD: 1.2<br /> <input name="pt" type="text"><input type="submit" value="Leech" ></form>';
}
elseif ($_POST['submit']){
...
echo $post;
echo '<u>Tiếp tục</u>:<br /><form method="get">Url: <input name="url" type="text"><br />(Dành cho truyện)Bỏ trống để leech full truyện. nếu muốn leech từng trang thì nhập VD: 1.2<br /> <input name="pt" type="text"><input type="submit" value="Leech" ></form>';
}
else
{
$url = $_GET['url'];
...
$lay = trim($lay);
if (!$lay){
$lay = explode('<b class="page">1</b>',$nd);
$lay = explode('»</a>',$lay[1]);
$lay = strip_tags($lay[0]);
$lay = trim($lay);
$lay = explode(' ',$lay);
...
if ($kt == 1){$cuoi = '<div style="';}
else {$cuoi = '<div class="topmenu"';}
}
echo '<b><u>'.$thong.'</u></b>
<form method="post">
...
$bai = curl_exec($bv);
$bai = explode('<div class="forumtxt">',$bai);
$bai = explode($cuoi,$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai,'<br>,<b>,<i>,<u>,<strong>,<a>,<img>');
$bai = preg_replace('#<img(.*?)src="(.*?)"(.*?)/>#i',"[img]$2[/img]",$bai);
$bai =  str_replace('/img.php?img=','',$bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
