<title>Leech truyentranhgay.com/</title>
<?php
session_start();
require 'head.php';
...
$title = trim($title[0]);
$lay =  str_replace('<ul class="list-group">','<div class="col-md-12 text-center manga-pages-container">' ,$lay);
$lay = explode('<div class="col-md-12 text-center manga-pages-container">',$lay);
$lay = explode('</div>',$lay[1]);
$lay = strip_tags($lay[0],'<img>,<br>');
$lay= preg_replace('#(.*?)<img(.*?)src="(.*?)"(.*?)/>(.*?)#is',"[img]$3[/img]\n",$lay);
$lay =  str_replace('[img]http://i.imgur.com/JrZyBoe.jpg[/img]','' ,$lay);
$lay = trim($lay);
curl_close($curl);
