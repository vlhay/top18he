...
$title = trim($title[0]);
$lay = curl_exec($curl);
$lay = explode('<div class="forumtxt"><big>',$lay);
$lay = explode('</big></div>',$lay[1]);
$lay = trim($lay[0]);
$lay = strip_tags($lay,'<img>,<br>');
$lay= preg_replace('#<img(.*?)src="(.*?)"(.*?)/>#is',"[img]$2[/img]",$lay);
$lay = trim($lay);
$lay= preg_replace('#/xemsex.mobi/#i',"hehe9x",$lay);
$lay= preg_replace('/xemsex.mobi/i',"",$lay);
$lay =  str_replace('hehe9x','xemsex.mobi',$lay);
curl_close($curl);
