<title>Tool Leech Blogtamsu.vn</title>
<?php
session_start();
require 'head.php';
...
$title = explode('|',$title);
$title = trim($title[0]);
$lay = explode('<div class="remain_detail">',$lay);
$lay = explode('<center><ins class="adsbygoogle"',$lay[1]);
$lay = trim($lay[0]);
$lay = strip_tags($lay,'<img>,<p>,<strong>,<em>,<div>');
$lay = trim($lay);
$lay = str_replace('(adsbygoogle = window.adsbygoogle || []).push({});','',$lay);
$lay = preg_replace('/\s\s+/', ' ', trim($lay));
curl_close($curl);
