...
$lay = explode('<div class="page-link">',$title);
...
$bai = curl_exec($bv);
$bai = explode('<div class="content">',$bai);
$bai = explode('</div>',$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai,'<p>,<br>,<b>,<i>,<u>,<strong>,<img>,<object>,<param>,<embed>');
$bai = preg_replace('/<img(.*?)src="(.*?)"(.*?)>/i','[img]$2[/img]',$bai);
$bai = preg_replace('/(tophinh|tophinh.com)/i',$_SESSION['domain'],$bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
