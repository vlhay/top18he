<title>Leech Truyen.wapchoi.mobi</title>
<?php
session_start();
require 'head.php';
...
{
echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><form method="get">Url: <input name="url" type="text"><br />Bỏ trống để leech hết truyện. Phân trang đầu và kết thúc cách nhau dấu chấm vd:"0.3" <br /> <input name="pt" type="text"><input type="submit" value="Leech" ></form>';
}
elseif ($_POST['submit']){
...
echo $post;
echo '<u>Tiếp tục</u>:<br /><form method="get">Url: <input name="url" type="text"><br />Bỏ trống để leech hết truyện. Phân trang đầu và kết thúc cách nhau dấu chấm vd:"0.3" <br /> <input name="pt" type="text"><input type="submit" value="Leech" ></form>';
}
else
{
$url = $_GET['url'];
...
$lay = trim($lay);
if (!$lay){
$lay = explode('<div class="page">1',$title);
$lay = explode('ĐỌC TIẾP</a>',$lay[1]);
$lay = strip_tags($lay[0]);
$lay = trim($lay);
$lay = explode(' ',$lay);
$lay = array_pop($lay);
...
$bai = curl_exec($bv);
$bai = explode('<div class="content-comic" style="margin:10px;">',$bai);
$bai = explode('</div>',$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai,'<br>,<b>,<i>,<u>,<strong>');
$bai = preg_replace('#truyen.wapchoi.mobi|wapchoi.mobi#i',"",$bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
