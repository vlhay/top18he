<title>Leech truyenvip.pro</title>
<?php
session_start();
require 'head.php';
...
$lay = explode('">›</a> <a class="last" href="'.$url.'/',$title);
$lay = explode('">»</a></div>',$lay[1]);
$lay = trim($lay[0]);
if (!$lay){
$lay = explode('<div class="wp-pagenavis">',$title);
$lay = explode('</a></div>',$lay[1]);
...
if($kt == 1)
{
$cuoi = '<div class="chiase">';
}
else {
$cuoi = '<div class="wp-pagenavis">';
}
$bv = curl_init();
for ($i= $bd; $i <= $kt ; $i++) { 
...
$bai = curl_exec($bv);
$bai = explode('<div class="listcontent">',$bai);
