...
$url = $_GET['url'];
$curl = curl_init();
curl_setopt ($curl, CURLOPT_URL, 'http://xomtruyen.com/naruto-chap-'.$url.'-tiengviet');
...
$lay =  str_replace('imgmax=2000','imgmax=2000" alt="truyen naruto tap '.$url.'"',$lay);
$lay =  str_replace("'",'"',$lay);
$lay = trim($lay);
curl_close($curl);
echo '<form method="post">
* Tiêu đề:<br/><input type="text" name="title" id="title" value="[Truyện tranh] Naruto chap '.$url.'" required=""/><br/>
URL:<br/> <input type="text" name="url" id="url" value=""/><br />
* Nội dung:<br/><textarea name="content">'.$lay.'</textarea><br/>
Từ khóa:(Cách nhau dấu ",")<br/>
<input type="text" name="tags" value="doc truyen tranh naruto, truyen naruto chap '.$url.', naruto tap '.$url.'"/><br/>
Ảnh đại diện: <br/><input type="text" name="img_thumb" value="http://i.imgur.com/7GEiklX.jpg"/><br/>
Title trên google:<br/><input type="text" name="title_seo" value=""/><br/>
Link tải:(Dùng HTML)<br/>
<textarea name="link_down" id="editor">Masashi Kishimoto</textarea><br/>
Mô tả:<br/><input type="text" name="desc" value="doc truyen tranh naruto, truyen naruto chap '.$url.', naruto tap '.$url.'"/><br/>
Mục hiển thị:<br />
'.$_SESSION['cm'].'
