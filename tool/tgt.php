<title>Leech thegioitruyen.mobi</title>
<?php
session_start();
require 'head.php';
...
{
$url = $_GET['url'];
$url =  str_replace('http://thegioitruyen.mobi/','',$url);
$url =  str_replace('/','',$url);
$url =  str_replace($url,'http://thegioitruyen.mobi/'.$url,$url);
$curl = curl_init();
...
$lay = explode('<div class="paging">',$title);
...
$lay = array_pop($lay);
if (!$lay){$lay = 1;}
$title = explode('<h1 itemprop="name">',$title);
...
$bai = curl_exec($bv);
$bai = explode('<div id="content" class="pad10 bderTopApk lineH20">',$bai);
$bai = explode('<div class="paging">',$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai,'<p>,<br>,<b>,<i>,<u>,<strong>');
$bai = preg_replace('#thegioitruyen.mobi#i',$_SESSION['domain'],$bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
