<title>Leech Kenhtruyen.biz</title>
<?php
session_start();
require 'head.php';
...
$pt = explode('...</div><div class="pagea">',$title);
$pt = explode('</div>',$pt[1]);
$pt = strip_tags($pt[0]);
$pt = trim($pt);
if(!$pt)
{
    $pt = explode('<div class="pagea">',$title);
    $pt = explode('</div>',$pt[1]);
    $pt = strip_tags($pt[0]);
    $pt = trim($pt);
}
...
for ($i= $bd; $i <= $kt ; $i++) { 
$url = preg_replace('#http://kenhtruyen.biz/bai-viet/(.*?)/(.*?)/(.*?)#i','http://kenhtruyen.biz/bai-viet/$1/'.$i.'/$3',$url);
curl_setopt ($bv, CURLOPT_URL,$url);
curl_setopt ($bv, CURLOPT_RETURNTRANSFER, 1);
$bai = curl_exec($bv);
$bai = explode('<div class="noidung">Nội Dung</div>',$bai);
$bai = explode('<div class="page">',$bai[1]);
$bai = trim($bai[0]);
$bai = strip_tags($bai);
$bai = preg_replace('#(http://kenhtruyen.biz|kenhtruyen.biz)#i',$_SESSION['domain'],$bai);
echo $bai;
}
curl_close($bv);
echo '</textarea><br/>
